          ...
          <div class="student-info-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item" v-if="submission.grade !== null">
                ...
                <span class="value">{{ submission.grade }}/{{ assignment.total_points }}</span>
              </div>
            </div>
          </div>
  ...
  methods: {
    async fetchSubmissionDetails() {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/api/assignment-submission/${submissionId}`);
        this.submission = response.data;
        this.submission.status = this.submission.grade !== null ? 'Graded' : 'Submitted';
        
        // Fetch assignment details
        const assignmentResponse = await axios.get(`http://127.0.0.1:8000/api/assignments/item/${this.submission.assignment_id}`);
    ...
    async submitGrade() {
      if (!this.grade || this.grade < 0 || this.grade > this.assignment.total_points) {
        const toast = useToast();
      ...
      try {
        const response = await axios.put(
          `http://127.0.0.1:8000/api/assignment-submission/${this.submission.submission_id}/grade`,
          {
            grade: this.grade,
    ...
    getFileUrl(filePath) {
      ...
      return `http://127.0.0.1:8000/api/assignments/download/${encodeURIComponent(fileName)}`;
    },
    ...
    downloadFile(filePath) {
      ...
      const encodedFilePath = encodeURIComponent(filePath.replace(/\\/g, '/')); // Ensure proper encoding and replace backslashes
      const downloadUrl = `http://127.0.0.1:8000/api/assignments/download/${encodedFilePath}`;
