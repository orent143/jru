...
export default {
  ...
  methods: {
    ...
    async saveEvent() {
      ...
      try {
        if (this.isEditingEvent) {
          ...
          await axios.put(`http://127.0.0.1:8000/api/events/${this.eventForm.id}`, this.eventForm);
          toast.success('Event updated successfully');
        } else {
          // Create new event
          await axios.post('http://127.0.0.1:8000/api/events/', this.eventForm);
    ...
    viewSchedule() {
      // Navigate to schedule view or implement schedule viewing logic
